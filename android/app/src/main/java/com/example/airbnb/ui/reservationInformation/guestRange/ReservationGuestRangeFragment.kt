package com.example.airbnb.ui.reservationInformation.guestRange
...
class ReservationGuestRangeFragment : Fragment() {
...
@Composable
fun GuestRangeView(viewModel: ReservationInformationViewModel) {
    Surface(
        modifier = Modifier
            .fillMaxSize(),
        ...
        Column {
            val adultCount = viewModel.adultCountStateFlow.collectAsState().value
            val childCount = viewModel.childCountStateFlow.collectAsState().value
            val toddlerCount = viewModel.toddlerCountStateFlow.collectAsState().value
            GuestInputHeader(
                input = "인원 입력",
                guest = "게스트 ",
                child = "유아 ",
            ...
            Column(
                ...
                GuestChildSelectorLayout(childCount) {
                    if (it > 0) {
                        viewModel.setDeleteFlagTrue()
                ...
                GuestToddlerSelectorLayout(toddlerCount) {
                    if (it > 0) {
                        viewModel.setDeleteFlagTrue()
                        //성인이 0명인 상태에서 증가버튼이 눌렸을때
...
fun GuestSelector(
    ...
    ageRange: String,
    countNumber: Int,
    onCountChanged: (countNumber: Int) -> Unit
) {
...
@Composable
fun GuestAdultSelectorLayout(countNumber: Int, onCountChanged: (countNumber: Int) -> Unit) {
    GuestSelector(type = "성인", ageRange = "만13세이상", countNumber, onCountChanged)
}

@Composable
fun GuestChildSelectorLayout(countNumber: Int, onCountChanged: (countNumber: Int) -> Unit) {
    GuestSelector(type = "어린이", ageRange = "만2~13세", countNumber, onCountChanged)
}

@Composable
fun GuestToddlerSelectorLayout(countNumber: Int, onCountChanged: (countNumber: Int) -> Unit) {
